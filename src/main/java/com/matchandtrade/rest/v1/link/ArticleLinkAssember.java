...
public class ArticleLinkAssember {
	...
	private static Set<Link> buildLink(Integer tradeMembershipId, Integer articleId) {
		...
		result.add(linkTo(methodOn(ArticleAttachmentController.class).get(tradeMembershipId, articleId, null, null)).withRel("attachments"));
		return result;
	}
	
	public static void assemble(ArticleJson json, Integer tradeMembershipId) {
		if (json != null) {
			json.getLinks().addAll(buildLink(tradeMembershipId, json.getArticleId()));
		}
	}

	public static void assemble(SearchResult<ArticleJson> response, Integer tradeMembershipId) {
